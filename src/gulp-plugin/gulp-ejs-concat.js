...
module.exports = function (file, opt) {
    if (!file) {
        throw new Error('gulp-ejs-concat: Missing file option');
    }
    opt = opt || {};
    ...
    function bufferContents(file, enc, cb) {
        ...
        //  添加处理方法，可以修改内容，参数为文件名和文件内容
        var contents = file.contents;
        if (opt.transform) {
            contents = opt.transform(file.relative, file.contents);
        }

        // add file to concat instance
        concat.add(file.relative, contents, file.sourceMap);
        cb();
    }
